...
Три блоки, рівно в цьому порядку, разом ≤ 20 рядків. Емодзі в заголовках і на
обовʼязкових рядках не прикраса, а навігація: власник читає звіт між справами й має
впізнати структуру, ще не читаючи тексту. Багатослівність тут є дефектом.

- `## 🎯 Підсумок` — ПЕРШЕ слово після заголовка це вирок: `ВИРІШЕНО`, `ЧАСТКОВО`,
  ...
  `⚠️ Ризик:` — лише коли ризик реальний.
- `## 🛠 Що зробив` — до 5 пунктів, кожен один рядок: зміна і навіщо вона. Далі три
  ...
  Завершує блок ОКРЕМИМ абзацом рядок `📦 Коміт:` — hash, повідомлення, факт пушу і
  стан CI; нічого не змінювалось — рівно `📦 Коміт: не потрібен — файли не змінювались`.
  Лишилось незакомічене — назвати що саме і чому; вирок тоді не `ВИРІШЕНО`, крім
  випадку, коли це чужі зміни паралельної сесії.
- `## ➡️ Що далі` — кожен рядок із маркером `[потрібно]` або `[опційно]`. Мету
