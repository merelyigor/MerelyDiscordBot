...
Фінал відповіді — два блоки: `## Звіт`, останнім `## Що далі`. Обидва пишуться для
власника як для людини, а не як технічний протокол.

1. **Зроблено** — 2-5 речень простою мовою: що змінилось і що це дає. Не журнал дій
   агента; шляхи, команди та жаргон — лише там, де без них незрозуміло.
2. **Зʼясовано** — лише висновки, що впливають на рішення власника; кожен одним
   реченням із доказом (цифра, вивід команди або рядок коду). Таких висновків немає —
   блок пропустити. Виправлення власної хибної заяви — окремим рядком, і тоді блок
   обовʼязковий.
3. **Перевірки** — пропорційні зміні: вузька перевірка доводить саме цю зміну, повний
   gate категорії запускається один раз перед фіналом, а не після кожної мікро-правки.
   Вказувати команду й результат; невиконані — окремо, з причиною та ризиком. Далі
   `SEO-check` і `OSM-check`.
4. `## Що далі` при повністю досягнутій меті без реальних ризиків — рівно один рядок:
   «Мету досягнуто, додаткова робота не потрібна.» Вигадані варіанти, поради та
   покращення «поки ми тут» заборонені: вони створюють власнику зайву роботу.
5. Інакше `## Що далі` називає, що лишилось, наступний конкретний крок, потребу від
   власника і Done criteria. Кожен пункт має маркер `[потрібно]` (без цього мета не
   досягнута або лишається реальний ризик) або `[опційно]` (можна ігнорувати без
   наслідків); пункт без маркера заборонений.
6. Заборонено переносити у «Що далі» роботу, яка входить у поточну задачу: усе
   потрібне для Definition of Done робиться зараз.
