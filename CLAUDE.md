...
- `## 🔧 Що змінилось` — до 4 пунктів «що змінилось → що це дає». Далі `✅ Перевірено:`
  словами: що доведено і чим; перевірку, яку треба було зробити й не зроблено, називай
  завжди з ризиком. `🔎 SEO-check:` і `🗺 OSM-check:` — лише коли торкався публічних
  сторінок, адрес, мета-тегів чи карт; «не застосовно» не писати. Завершує блок ОКРЕМИМ
  абзацом `📦 Коміт:` — хеш, про що, факт пушу, стан CI; змін не було — рівно
