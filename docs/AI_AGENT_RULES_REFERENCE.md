...
- §13.1 `git commit` агент робить сам, а `git push` — ні: відправляє власник власноруч або агент за разовим явним дозволом: за конвенцією повідомлення цього репозиторію, стейджачи лише свої шляхи, після гейта з кодом 0. Force-push, rebase, теги й будь-яке переписування історії — лише за разовим явним дозволом (рішення власника 2026-09-18).
...
- §15.4 Рівно три блоки в цьому порядку: `## 🎯 Результат` → `## 🔧 Що змінилось` → `## ➡️ Що потрібно від тебе`. Адресат — власник, який не відкривав код.
- §15.5 ПЕРШЕ слово `## 🎯 Результат` — вирок: `ВИРІШЕНО`, `ЧАСТКОВО`, `НЕ ВИРІШЕНО`, `ЗАБЛОКОВАНО`, `ВІДПОВІДЬ` (задача без змін). Синоніми, емодзі замість слова й «в основному зроблено» заборонені.
- §15.6 `## 🔧 Що змінилось` — до 4 пунктів «що змінилось → що це дає», далі `✅ Перевірено:` словами і ОКРЕМИМ абзацом `📦 Коміт:`. Рядків `🔎 SEO-check:` і `🧱 OSM-check:` тут не буває: публічних сторінок і карт проєкт не має, а «не застосовно» писати заборонено.
- §15.7 Виправлення власної хибної заяви — окремий рядок у `## 🎯 Результат`.
- §15.8 Фінал фіксує результат, а не процес: що читав, шукав і які гіпотези відкинув — не пишеться.
- §15.9 Незавершена задача — що лишилось, наступний конкретний крок, потреба від власника і Done criteria.
- §15.10 Завершена задача — останній блок містить рівно один рядок «Нічого — задачу завершено.»
- §15.11 Кожен пункт останнього блоку має маркер `[потрібно]` або `[опційно]`; `[опційно]` не більше одного.
- §15.12 Роботу поточної задачі в останній блок не переносити.
- §15.13 `🔍 Причина:` — ЛИШЕ для поламки чи розслідування: корінь словами, доказ у дужках. Не доведена — `🔍 Ймовірна причина:` зі словом «гіпотеза» (рішення власника 2026-09-18).
- §15.14 `## 🎯 Результат` самодостатній: стан, суть проблеми і ризик видно з нього одного.
- §15.15 Вирок `ВИРІШЕНО` заборонений, якщо релевантний gate не завершився з кодом 0 або перевірка не запускалась.
- §15.16 Мова звіту [MUST]: пояснювати ефект, а не реалізацію. Назва файла, класу, функції чи гейта — лише коротким доказом у дужках після речення, яке вже все пояснило словами. Скорочення — лише ті, якими користується власник. Одна думка — один пункт; жорсткої межі в рядках немає (рішення власника 2026-09-18).
