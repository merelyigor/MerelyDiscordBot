...
- §15.4 Фінал: `## Звіт` (**Зроблено** → **Зʼясовано** → **Перевірки** → SEO-check → OSM-check) → `## Що далі`. Обидва блоки пишуться для власника як для людини, а не як технічний протокол.
- §15.5 `## Звіт` самодостатній. **Зроблено** — 2-5 речень простою мовою: що змінилось і що це дає, а не журнал дій агента.
- §15.6 **Перевірки** пропорційні зміні: вузька перевірка доводить саме цю зміну, повний gate — один раз перед фіналом. Вказувати команду й результат; невиконані — окремо з причиною та ризиком.
- §15.7 Виправлення власної хибної заяви — окремий рядок у **Зʼясовано**; у цьому випадку блок обовʼязковий.
...
- §15.13 **Зʼясовано** містить лише висновки, що впливають на рішення власника, кожен одним реченням із доказом. Таких висновків немає — блок пропустити.
