...
- §15.4 Фінал відповіді — рівно три блоки в цьому порядку: `## Підсумок` → `## Що зробив` → `## Що далі`. Разом ≤ 20 рядків. Пишуться для власника як для людини, а не як технічний протокол.
- §15.5 ПЕРШЕ слово `## Підсумок` — вирок, і лише один із цих: `ВИРІШЕНО`, `ЧАСТКОВО`, `НЕ ВИРІШЕНО`, `ЗАБЛОКОВАНО` (потрібне рішення або дозвіл власника), `ВІДПОВІДЬ` (задача без змін: аналіз, розвідка, питання). Далі одне речення що саме. Синонімів, емодзі замість слова та формулювань на кшталт «в основному зроблено» не використовувати.
- §15.6 Структура `## Що зробив`: до 5 пунктів, кожен один рядок (зміна і навіщо вона), далі три рядки, присутні ЗАВЖДИ: `Перевірка:` (команда → результат; не виконана — так і написати, з ризиком), `SEO-check:`, `OSM-check:`. Для worker-проєкту без UI і URL це `не застосовно`, але рядок лишається, бо власник переглядає його окремо від решти.
- §15.7 Виправлення власної хибної заяви — окремий рядок у `## Підсумок`.
- §15.8 Аналіз над фіналом короткий; фінал фіксує результат, а не переказує процес: що читав, шукав, які гіпотези відкинув — не пишеться.
- §15.9 `## Що далі` для незавершеної задачі: що лишилось, наступний конкретний крок, потреба від власника і Done criteria.
- §15.10 Мета досягнута повністю і реальних ризиків немає — «Що далі» містить рівно один рядок «Мету досягнуто, додаткова робота не потрібна.» Вигадувати варіанти, поради чи покращення «поки ми тут» заборонено.
- §15.11 Кожен пункт у «Що далі» має маркер `[потрібно]` (без цього мета не досягнута або лишається реальний ризик) або `[опційно]` (можна ігнорувати без наслідків). Пункт без маркера заборонений.
- §15.12 Заборонено переносити у «Що далі» роботу, яка входить у поточну задачу; усе потрібне для Definition of Done робиться зараз.
- §15.13 Другий рядок `## Підсумок` — обовʼязковий `Причина:` — корінь проблеми з доказом у дужках (`файл:рядок`, вивід команди або рядок логу). Причину не встановлено — писати рівно `Причина: не встановлена` і що саме лишилось невідомим. Здогад подавати лише зі словом `гіпотеза`; вирок без доказу причини заборонений, бо на ньому власник ухвалює рішення.
- §15.14 `## Підсумок` самодостатній: прочитавши лише його, власник знає стан задачі, у чому була проблема і чи лишився ризик. Решта блоків — для того, хто хоче деталей.
- §15.15 Вирок `ВИРІШЕНО` заборонений, якщо релевантний gate не завершився з кодом 0 або перевірка не запускалась: у такому разі вирок `ЧАСТКОВО` чи `НЕ ВИРІШЕНО` з рядком `Ризик:`, що саме не доведено.
